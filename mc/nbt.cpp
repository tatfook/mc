...
namespace mapcrafter {
	namespace mc {
		namespace nbt {
			...
			NBTTag& TagLongArray::read(std::istream& stream) {
				int length = ((TagInt&)TagInt().read(stream)).payload;
				for (int i = 0; i < length; i++) {
					int64_t element = 0;
					stream.read((char*)&element, sizeof(element));
					element = be64toh(element);
					payload.push_back(element);
				}
				return *this;
			}

			void TagLongArray::write(std::ostream& stream) const {
				NBTTag::write(stream);
				TagInt length(payload.size());
				length.write(stream);
				TagLong element;
				element.setWriteType(false);
				for (std::vector<int64_t>::const_iterator it = payload.begin(); it != payload.end();
					++it) {
					element.payload = *it;
					element.write(stream);
				}
			...
			void TagLongArray::dump(std::ostream& stream, const std::string& indendation) const {
				stream << indendation << "TAG_LongArray";
				if (named) {
					stream << "(\"" << name << "\")";
				}
				stream << ": " << payload.size() << " entries" << std::endl;
			...
			NBTTag& TagShortArray::read(std::istream& stream) {
				int length = ((TagInt&)TagInt().read(stream)).payload;
				for (int i = 0; i < length; i++) {
					int16_t element = 0;
					stream.read((char*)&element, sizeof(element));
			...
			void TagShortArray::write(std::ostream& stream) const {
				NBTTag::write(stream);
				TagInt length(payload.size());
				length.write(stream);
				TagLong element;
				element.setWriteType(false);
				for (std::vector<int16_t>::const_iterator it = payload.begin(); it != payload.end();
					++it) {
					element.payload = *it;
					element.write(stream);
				}
			...
			void TagShortArray::dump(std::ostream& stream, const std::string& indendation) const {
				stream << indendation << "TAG_ShortArray";
				if (named) {
					stream << "(\"" << name << "\")";
				}
				stream << ": " << payload.size() << " entries" << std::endl;
